...
class AuthPermTestCase(TestCase):
    ...
    def test_get_with_anonymous_user(self):
        resp = self.anonymous_client.get("/api/cables/poles/")
        self.assertEquals(resp.status_code, 401)
        resp = self.anonymous_client.get("/api/cables/poles-full/")
        self.assertEquals(resp.status_code, 401)

    def test_post_with_anonymous_user(self):
        resp = self.anonymous_client.post("/api/cables/poles/", data={})
        self.assertEquals(resp.status_code, 401)

    def test_put_with_anonymous_user(self):
        resp = self.anonymous_client.put("/api/cables/poles/", data={})
        self.assertEquals(resp.status_code, 401)

    def test_patch_with_anonymous_user(self):
        resp = self.anonymous_client.patch("/api/cables/poles/", data={})
        self.assertEquals(resp.status_code, 401)

    def test_delete_with_anonymous_user(self):
        resp = self.anonymous_client.delete("/api/cables/poles/")
        self.assertEquals(resp.status_code, 401)

    def test_get_with_unauthentified_user(self):
        resp = self.unauthentified_client.get("/api/cables/poles/")

        self.assertEquals(resp.status_code, 401)
        resp = self.unauthentified_client.get("/api/cables/poles-full/")
    ...
    def test_post_with_unauthentified_user(self):
        resp = self.unauthentified_client.post("/api/cables/poles/", data={})
        self.assertEquals(resp.status_code, 401)

    def test_put_with_unauthentified_user(self):
        resp = self.unauthentified_client.put("/api/cables/poles/", data={})
        self.assertEquals(resp.status_code, 401)

    def test_patch_with_unauthentified_user(self):
        resp = self.unauthentified_client.patch("/api/cables/poles/", data={})
        self.assertEquals(resp.status_code, 401)

    def test_delete_with_unauthentified_user(self):
        resp = self.unauthentified_client.delete("/api/cables/poles/")
        self.assertEquals(resp.status_code, 401)

    def test_get_with_unauthorized_user(self):
        # no restriction for view based on django permission
        resp = self.unauthorized_client.get("/api/cables/poles/")
        self.assertEquals(resp.status_code, 200)
        resp = self.unauthorized_client.get("/api/cables/poles-full/")
        self.assertEquals(resp.status_code, 200)

    def test_post_with_unauthorized_user(self):
        resp = self.unauthorized_client.post("/api/cables/poles/", data={})
        self.assertEquals(resp.status_code, 403)

    def test_put_with_unauthorized_user(self):
        resp = self.unauthorized_client.put("/api/cables/poles/", data={})
        self.assertEquals(resp.status_code, 403)

    def test_patch_with_unauthorized_user(self):
        resp = self.unauthorized_client.patch("/api/cables/poles/", data={})
        self.assertEquals(resp.status_code, 403)

    def test_delete_with_unauthorized_user(self):
        resp = self.unauthorized_client.delete("/api/cables/poles/", data={})
        self.assertEquals(resp.status_code, 403)
