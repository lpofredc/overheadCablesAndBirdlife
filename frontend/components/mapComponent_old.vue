<template>
  <l-map ref="map" class="d-flex align-stretch" :class="editMode ? 'change-map' : 'view-map'" :center="center"
    ...
    <v-speed-dial class="fab mb-5" absolute bottom right direction="top" transition="slide - y - reverse - transition">
      ...
      <v-btn fab dark size="small" color="orange">
        <v-icon>mdi-shape-polygon-plus</v-icon>
      </v-btn>
      <v-btn fab dark size="small" to="/supports/new" color="green">
        <v-icon>mdi-transmission-tower</v-icon>
      </v-btn>
      <v-btn fab dark size="small" to="/lines/new" color="indigo">
        <v-icon>mdi-cable-data</v-icon>
      </v-btn>
      <v-btn fab dark size="small" to="/mortality/new" color="red">
...
const newLineMarkers = reactive([[45, 7]])
const map = ref(null)
const createLayer = ref(null)
const bounds = reactive(latLngBounds([
  [40, -6],
  [52, 10]
]))
const maxBounds = reactive(latLngBounds([
  [40, -6],
  [52, 10]
]))
const zoom = ref(5)
const path = ref('/images/')
const center = reactive([47.41322, -1.219482])
...
const GeojsonOptions = computed(() => {
  return {
     ...
     */
    const GeoJsonOnEachFeature = computed(() {
