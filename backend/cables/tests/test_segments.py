...
class SegmentAnonymousAuthenticationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.pk = Segment.objects.all()[0].id

    def test_get_list_with_anonymous_user(self):
        resp = self.anonymous_client.get("/api/v1/cables/segments/")
        self.assertEquals(resp.status_code, 401)
        resp = self.unauthentified_client.get("/api/v1/cables/segments/")
        self.assertEquals(resp.status_code, 401)

    def test_get_detail_with_anonymous_user(self):
        resp = self.anonymous_client.get(f"/api/v1/cables/segments/{self.pk}/")
        self.assertEquals(resp.status_code, 401)
        resp = self.unauthentified_client.get(
            f"/api/v1/cables/segments/{self.pk}/"
        )
        self.assertEquals(resp.status_code, 401)

    def test_create_with_anonymous_user(self):
        resp = self.anonymous_client.post("/api/v1/cables/segments/")
        self.assertEquals(resp.status_code, 401)
        resp = self.unauthentified_client.post("/api/v1/cables/segments/")
        self.assertEquals(resp.status_code, 401)

    def test_update_with_anonymous_user(self):
        resp = self.anonymous_client.put(f"/api/v1/cables/segments/{self.pk}/")
        self.assertEquals(resp.status_code, 401)
...
class SegmentUnauthorizedAuthenticationTestCase(TestCase):
    """Class to test authentication/permission scheme for Segment and unauthorized user."""
    ...
    def test_get_list_with_unauthorized_user(self):
        resp = self.unauthorized_client.get("/api/v1/cables/segments/")
    ...
    def test_create_with_unauthorized_user(self):
        resp = self.unauthorized_client.post("/api/v1/cables/segments/")
